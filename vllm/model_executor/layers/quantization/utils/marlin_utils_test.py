...
def marlin_int2_weights(q_w, size_k, size_n, perm):
    # Permute
    q_w = marlin_permute_weights(q_w, size_k, size_n, perm)

    num_bits = 2
    # Pack
    pack_factor = 16 // num_bits
    orig_device = q_w.device

    q_w = q_w.cpu().numpy().astype(np.uint16)
    ...
    q_packed = torch.from_numpy(q_packed.astype(np.int16)).to(orig_device)
...
def get_int2_weight_perm():
    ...
    interleave = np.array([0, 1, 2, 3, 4, 5, 6, 7])
...
def marlin_int2_quantize(w: torch.Tensor,
                         quant_type: ScalarType,
                         group_size: int):
    size_k, size_n = w.shape
    ...
    w_ref, q_w, s, _, _ = gptq_quantize_weights(
        w, quant_type, group_size, False, None)

    # Reformat to marlin
    weight_perm = get_int2_weight_perm()
    marlin_q_w = marlin_int2_weights(q_w, size_k, size_n, weight_perm)
    marlin_s = marlin_permute_scales(s, size_k, size_n, group_size)
...
def awq_marlin_int2_quantize(w: torch.Tensor, quant_type: ScalarType,
                             group_size: int):
    size_k, size_n = w.shape
