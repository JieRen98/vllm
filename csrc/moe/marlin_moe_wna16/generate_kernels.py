...
def generate_new_int2_kernels():
    FILE_HEAD = """
    // auto generated by generate.py
    // clang-format off
    
    #include "kernel.h"
    #include "marlin_int2_template.h"
    
    namespace MARLIN_NAMESPACE_NAME {
    """.strip()

    TEMPLATE = ("template __global__ void MarlinInt2<"
                "{{scalar_t}}, "
                "{{w_type_id}}, "
                "{{threads}}, "
                "{{thread_m_blocks}}, "
    ...
    for scalar_type, dtype in itertools.product(SCALAR_TYPES, DTYPES):
        ...
        file_content = FILE_HEAD + "\n\n"
        file_content += "\n\n".join(all_template_str_list) + "\n\n}\n"
        filename = f"int2_kernel_{dtype}_{scalar_type[6:].lower()}.cu"
